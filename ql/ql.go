...
var NotSelectStatementError = errors.New("not SelectStatement")
...
func Parse(ql string) (*influxql.SelectStatement, error) {
	st, err := influxql.ParseStatement(ql)
	if err != nil {
		return nil, err
	}
	ss, ok := st.(*influxql.SelectStatement)
	if !ok {
		return nil, NotSelectStatementError
	}

	return ss, nil
}

func Refine(ss *influxql.SelectStatement) (*compiledStatement, error) {
	s := ss.Clone()
	opt := CompileOptions{Now: time.Now().UTC()}
	cs, err := Compile(s, opt)
	if err != nil {
		return nil, err
	}
	if cs.TimeRange.MaxTimeNano() == influxql.MaxTime {
		cs.TimeRange.Max = opt.Now
	}
	if cs.TimeRange.MinTimeNano() == influxql.MinTime {
		cs.TimeRange.Min = time.Time{}
	}
	sort.Sort(SortableDimensions(cs.Stmt.Dimensions))
	sort.Sort(cs.Stmt.Fields)
	return cs, nil
}

func Hash(ss *influxql.SelectStatement) string {
	h := md5.New()
	io.WriteString(h, ss.String())
	return fmt.Sprintf("%x", h.Sum(nil))
}

type SortableDimensions influxql.Dimensions

func (a SortableDimensions) Len() int      { return len(a) }
func (a SortableDimensions) Swap(i, j int) { a[i], a[j] = a[j], a[i] }
func (a SortableDimensions) Less(i, j int) bool {
	x := a[i]
	y := a[j]
	switch expx := x.Expr.(type) {
	case *influxql.Call:
		switch expy := y.Expr.(type) {
		case *influxql.Call:
			return strings.Compare(expx.String(), expy.String()) < 0
		case *influxql.VarRef:
			return true
		}
	case *influxql.VarRef:
