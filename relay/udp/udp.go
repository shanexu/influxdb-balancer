...
func NewUDP(config relay.UDPConfig) (relay.Relay, error) {
	u := new(UDP)
	...
	for {
		...
		if err != nil {
			if atomic.LoadInt64(&u.closing) == 0 {
				log.Infof("Error reading packet in relay %q from %v: %v", u.name, remote, err)
			} else {
				err = nil
			}
			close(queue)
...
func (u *UDP) post(p *packet) {
	...
	if err != nil {
		log.Infof("Error parsing packet in relay %q from %v: %v", u.Name(), p.from, err)
		putUDPBuf(p.data)
		return
	}
	...
	if err != nil {
		putUDPBuf(out)
		log.Infof("Error writing points in relay %q: %v", u.Name(), err)
		return
	}

	for _, b := range u.backends {
		if err := b.post(out.Bytes()); err != nil {
			log.Infof("Error writing points in relay %q to backend %q: %v", u.Name(), b.name, err)
		}
	}
